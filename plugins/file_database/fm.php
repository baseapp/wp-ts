...
if(isset($_REQUEST['file']) && strlen($_REQUEST['file']) > 0) {
	$tmp = str_replace('\\','/',realpath($_REQUEST['file']))."/";
	if ($tmp === false)
		err(404, 'File or Directory Not Found');
	if(substr($tmp, 0,strlen(TS_ABSPATH)) !== TS_ABSPATH)
		err(403,"Forbidden");
	...
	<script>
		...
		$(function(){
			...
			$('#mkdir').submit(function(e) {
				...
				e.preventDefault();
				$dir.val().length && $.post('?ts_plugin=file_database/fm',{'do':'mkdir',name:$dir.val(),xsrf:XSRF,file:hashval},function(data){
					list();
				},'json');
				$dir.val('');
				return false;
			...
			function uploadFile(file) {
				...
				xhr.open('POST', '?ts_plugin=file_database/fm');
				xhr.onload = function() {
					$row.remove();
					list();
				};
			...
			function list() {
				var hashval = window.location.hash.substr(1);
				$.get('?ts_plugin=file_database/fm',{'do':'list','file':hashval},function(data) {
					$tbody.empty();
			...
			function renderFileRow(data) {
				var $link = $('<a class="name" />')
					...
					.text(data.name);
				var $dl_link = $('<a/>').attr('href','?ts_plugin=file_database/fm&do=download&file='+encodeURIComponent(data.path))
